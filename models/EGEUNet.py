...
class EGEUNet(nn.Module):
    def __init__(self, num_classes=2, input_channels=3, c_list=[8, 16, 24, 32, 48, 64], bridge=True, gt_ds=True):
        super().__init__()
    ...
    def forward(self, x):
        ...
        out4 = F.gelu(F.interpolate(self.dbn2(self.decoder2(out5)), scale_factor=2, mode="bilinear", align_corners=True))
        if self.gt_ds:
        ...
        out3 = F.gelu(F.interpolate(self.dbn3(self.decoder3(out4)), scale_factor=2, mode="bilinear", align_corners=True))
        if self.gt_ds:
        ...
        out2 = F.gelu(F.interpolate(self.dbn4(self.decoder4(out3)), scale_factor=2, mode="bilinear", align_corners=True))
        if self.gt_ds:
        ...
        out1 = F.gelu(F.interpolate(self.dbn5(self.decoder5(out2)), scale_factor=2, mode="bilinear", align_corners=True))
        if self.gt_ds:
        ...
        if self.num_classes == 1:
            return torch.sigmoid(out0)  # 二分类
        else:
            return out0  # 多分类，未使用 softmax，交由损失函数处理